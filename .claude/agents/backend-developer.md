...
- **Middleware**: Better Auth session validation → inject `userId` into GraphQL context
...
- ❌ `/frontend/**` - Handled by Full-Stack Frontend Agent
- ❌ `/mobile/**` - Handled by Full-Stack Frontend Agent
- ❌ `/shared/**` - Coordinate with Full-Stack Frontend Agent first
...
- ✅ After completing each sub-task (e.g., Prisma schema done)
- ✅ After all tests for a feature pass (GREEN phase)
...
- ✅ Redis TTL set on all cache entries
