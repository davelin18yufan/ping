...
describe("GraphQL Integration", () => {
    ...
    `,
        ...
        expect(new Date(meData.createdAt).toISOString()).toBe(user.createdAt.toISOString())
        expect(new Date(meData.updatedAt).toISOString()).toBe(user.updatedAt.toISOString())
    })
    ...
    test("should reject expired session", async () => {
        ...
        const { sessionToken } = await createTestUserWithExpiredSession(testPrisma)
