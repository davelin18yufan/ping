...
describe("Socket.io Integration", () => {
  ...
  test("should connect authenticated user successfully", async () => {
    ...
    expect(authData.socketId).toBe(socket.id);
  ...
  test("should map userId to socketId in Redis", async () => {
    ...
    expect(socketIds).toContain(socket.id);
  ...
  test("should cleanup Redis on disconnect", async () => {
    ...
    const socketIds = await redis.smembers(`user:sockets:${user.id}`);
    expect(socketIds).not.toContain(socket.id);
  ...
  test("should support multiple sockets for same user", async () => {
    ...
    await waitForSocketConnect(socket1);
    await waitForSocketConnect(socket2);
    await waitForSocketEvent(socket1, "authenticated");
    await waitForSocketEvent(socket2, "authenticated");

    // Assert: Redis should have both socket IDs
    const socketIds = await redis.smembers(`user:sockets:${user.id}`);
    expect(socketIds).toContain(socket1.id);
    expect(socketIds).toContain(socket2.id);
    ...
    expect(remainingSockets).not.toContain(socket1.id);
    expect(remainingSockets).toContain(socket2.id);
